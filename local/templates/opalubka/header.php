...
<head>
	...
	<?$APPLICATION->AddHeadString('<script>BX.message('.CUtil::PhpToJSObject( $MESS, false ).')</script>', true);?>
        <?$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH."/css/blocks/custom.css", true)?>
	<?if($bIncludedModule)
		CMax::Start(SITE_ID);
	?>
    ...
    Asset::getInstance()->addJs('/lazyload.js');
		...
		<div class="wraps hover_<?=$arTheme["HOVER_TYPE_IMG"]["VALUE"];?>" id="content">
			...
			<?endif;?>
				...
				<?if(($isIndex && ($isShowIndexLeftBlock || $bActiveTheme)) || (!$isIndex && !$isHideLeftBlock)):?>
					<div class="<?if ($_SERVER['REQUEST_URI']!='/'):?>right_block<?endif;?> <?=(defined("ERROR_404") ? "error_page" : "");?> wide_<?=CMax::ShowPageProps("HIDE_LEFT_BLOCK");?> <?=$APPLICATION->ShowViewContent('right_block_class')?>">
				<?endif;?>
