										...
										<div class="img shine">
											<?if($bIcons && $arSection["UF_CATALOG_ICON"]):?>
												...
												<a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="thumb">
													...
													<?else:?>
														<img class="lazy" data-src="<?=$img["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($img["src"]);?>" alt="<?=($arSection["PICTURE"]["ALT"] ? $arSection["PICTURE"]["ALT"] : $arSection["NAME"])?>" title="<?=($arSection["PICTURE"]["TITLE"] ? $arSection["PICTURE"]["TITLE"] : $arSection["NAME"])?>" itemscope itemtype="http://schema.org/ImageObject"/>
														<meta itemprop="contentUrl" content="<?=$img["src"]?>">
														<meta itemprop="name" content="<?=($arSection["PICTURE"]["TITLE"] ? $arSection["PICTURE"]["TITLE"] : $arSection["NAME"])?>">
														<meta itemprop="description" content="<?=($arSection["PICTURE"]["ALT"] ? $arSection["PICTURE"]["ALT"] : $arSection["NAME"])?>">
													<?endif;?>
												</a>
											<?else:?>
												...
												<?else:?>
													<a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="thumb">
														<img class="lazy" data-src="<?=SITE_TEMPLATE_PATH?>/images/svg/noimage_product.svg" src="<?=\Aspro\Functions\CAsproMax::showBlankImg(SITE_TEMPLATE_PATH.'/images/svg/noimage_product.svg');?>" alt="<?=$arSection["NAME"]?>" title="<?=$arSection["NAME"]?>" height="90" itemscope itemtype="http://schema.org/ImageObject"/>
														<meta itemprop="contentUrl" content="<?=SITE_TEMPLATE_PATH?>/images/svg/noimage_product.svg">
														<meta itemprop="name" content="<?=$arSection["NAME"]?>">
														<meta itemprop="description" content="<?=$arSection["NAME"]?>">
													</a>
												<?endif;?>
											<?endif;?>
										</div>
											...
											<div class="section_info toggle">
												<ul>
													...
													<?if($arSection['ITEMS']):
														...
														foreach($arSection['ITEMS'] as $key => $arItem):?>
															<?$sectionName = ($arParams['SECTION_TYPE_TEXT'] == 'SEO' && $arItem['IPROPERTY_VALUES']['SECTION_PAGE_TITLE'] ? $arItem['IPROPERTY_VALUES']['SECTION_PAGE_TITLE'] : $arItem['NAME'])?>
															<li class="sect <?=(++$key > $iVisibleItemsMenu ? 'collapsed' : '');?> font_xs"><a href="<?=$arItem['SECTION_PAGE_URL']?>" class=""><?=$sectionName?><? echo $arItem['ELEMENT_CNT']?' <span>'.$arItem['ELEMENT_CNT'].'</span>':'';?><?if($key < $iCountChilds):?><span class="separator">—</span><?endif;?></a></li>
														<?endforeach;?>
