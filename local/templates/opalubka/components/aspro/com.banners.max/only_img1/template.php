									...
									<div id="<?=$this->GetEditAreaId($arItem['ID']);?>" class="item">
										<div class="img shine<?=($bBannerWithText ? ' pull-left' : '')?>">
											<div class="img_inner">
												...
												<?endif;?>
												<span class="lazy<?=($bBannerWithText ? ' rounded' : '')?><?=$position;?><?=(!$bWide && !$bBannerWithText ? ($arParams['SIZE_IN_ROW'] == 2 ? ' rounded4' : ' rounded3') : '');?>" data-src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($arItem['PREVIEW_PICTURE']['SRC']);?>)" itemscope itemtype="http://schema.org/ImageObject">
