						...
						<?foreach($arResult['ITEMS'] as $i => $arItem):?>
							...
							<div class="item-wrapper col-md-<?=(($arParams['SIZE_IN_ROW'] == 5) ? '2 col-m-20' : $col);?> col-sm-4 col-xs-6 clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
								<div class="item<?=($arParams['CENTERED'] == 'Y' && $arParams['TYPE_IMG'] == 'top' ? ' text-center' : '');?> <?=($bSmallBlock || $arParams['TYPE_IMG'] !== 'top' ? 'flexbox flexbox--row' : '');?>">
									<?if($bImage):?>
										<?$img = CFile::ResizeImageGet($bImage, array('width' => 125, 'height' => 125), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);?>
										<div class="image shine<?=($arItem['PROPERTIES']['COLORED']['VALUE'] == 'Y' ? ' colored_theme_svg' : '');?><?=($arParams['TYPE_IMG'] !== 'top' ? ' pull-'.$arParams['TYPE_IMG'] : '');?>" itemscope itemtype="http://schema.org/ImageObject">
											<?if($bDetailLink):?>
											...
											<?endif;?>
												...
												<?else:?>
													<img class="lazy" data-src="<?=$img["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($img["src"]);?>" alt="<?=($arItem["PREVIEW_PICTURE"]["ALT"] ? $arItem["PREVIEW_PICTURE"]["ALT"] : $arItem["NAME"])?>" title="<?=($arItem["PREVIEW_PICTURE"]["TITLE"] ? $arItem["PREVIEW_PICTURE"]["TITLE"] : $arItem["NAME"])?>" itemprop="contentUrl" />
												<?endif;?>
												<meta itemprop="name" content="<?=($arItem["PREVIEW_PICTURE"]["ALT"] ? $arItem["PREVIEW_PICTURE"]["ALT"] : $arItem["NAME"])?>">
