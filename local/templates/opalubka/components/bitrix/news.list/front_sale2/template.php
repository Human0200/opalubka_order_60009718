		...
		?>
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<div class="item-wrapper col-lg-<?=$col_lg;?> col-md-<?=$col_md;?> col-sm-6 col-xs-6 col-xxs-12 clearfix <?=($bSlider ? ' item-width-261' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					<?if($bFonImg):?>
					<div class="item box-shadow rounded3 darken-bg-animate lazy<?=$position;?>" <?=($bImage ? 'data-src="'.$imageSrc.'" style="background-image:url('.\Aspro\Functions\CAsproMax::showBlankImg($imageSrc).')" itemscope itemtype="http://schema.org/ImageObject"' : '');?>>
						<?if($bImage):?>
					...
					<div class="item<?=($arParams['FILLED'] == 'Y' ? ' bg-fill-grey' : ($arParams['TRANSPARENT'] == 'Y' ? '' : ' bg-fill-white'));?><?=($arParams['TRANSPARENT'] == 'Y' ? '' : ' box-shadow');?><?=($arParams['TYPE_IMG'] == 'sm' ? ' bordered text-center' : '');?>">
						<?if($bImage):?>
							<div class="image shine">
								...
								<?endif;?>
									<span class="rounded<?=($arParams['TYPE_IMG'] != 'sm' ? 3 : '');?> bg-fon-img lazy<?=$position;?>" data-src="<?=$imageSrc?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)" itemscope itemtype="http://schema.org/ImageObject">
										<meta itemprop="contentUrl" content="<?=$imageSrc?>">
										<meta itemprop="name" content="<?=$arItem['NAME']?>">
										<meta itemprop="description" content="<?=$arItem['NAME']?>">
									</span>
					...
					<?endif;?>
						<div class="inner-text">
							...
							<div class="title <?=($bFonImg ? 'font_mlg' : '');?>">
								<?if($bDetailLink):?>
									<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
								<?endif;?>
								<?=$arItem['NAME'];?>
