	...
	<?if(!$arParams['IS_AJAX']):?>
		...
		<div class="item-views news2 <?=$arParams['TYPE_IMG'];?><?=(!$bBordered ? '' : ' with-border');?><?=($arParams['HALF_BLOCK'] == 'Y' ? ' half-block' : '');?> <?=($bgSmallPlate ? 'small-bg-plate' : '');?> <?=$sTemplateMobile;?>">
			<div class="items<?=(!$arParams['INCLUDE_FILE'] ? '' : ' list');?> s_<?=$arParams['SIZE_IN_ROW'];?>">
				<div class="objects-slider-container flexbox <?=$sTemplateMobile;?><?=($bSlider ? ' swipeignore mobile-margin-16 mobile-compact' : '');?><?=$bHasBottomPager ? ' has-bottom-nav' : ''?> owl-carousel owl-theme owl-bg-nav" data-plugin-options='{"nav": false, "dots": true, "dotsEach": true, "loop": true, "marginMove": false, "autoplay": false, "useCSS": false, "responsive": {"0":{"items": 1, "autoWidth": false, "lightDrag": false, "margin":10, "autoplay": false}, "601":{"items": 2, "autoWidth": false, "lightDrag": false, "margin":10, "autoplay": false}, "1200":{"items": 3, "margin":20, "nav": true, "dots": false, "dotsEach": false}}}'>
	<?endif;?>
			<?
			$indexItem = 0;
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?endif;?>
					...
					<?if($bBgImage):?>
					<div class="item with-fon <?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-title-fon' : ' darken-bg-animate ');?> box-shadow rounded3  <?=($useImageAlternate ? ' bg-img-alternate ' : '');?> <?=($shortBigBlock && $bBigBlock ? ' short-big-block ' : '');?> <?=($bBigBlock ? 'big-block': ($arParams['TALL_BG_BLOCKS']=='Y'? 'tall-block':''));?> lazy<?=$position;?>" <?=($bImage ? 'data-src="'.$imageSrc.'" style="background-image:url('.\Aspro\Functions\CAsproMax::showBlankImg($imageSrc).')" itemscope itemtype="http://schema.org/ImageObject"' : 'data-src="'.$noImageSrc.'" style="background-image:url('.\Aspro\Functions\CAsproMax::showBlankImg($noImageSrc).')"');?> id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						<?if($bImage):?>
							<meta itemprop="contentUrl" content="<?=$imageSrc?>">
							<meta itemprop="name" content="<?=$arItem['NAME']?>">
							<meta itemprop="description" content="<?=$arItem['NAME']?>">
						<?else:?>
							<meta itemprop="contentUrl" content="<?=$noImageSrc?>">
							<meta itemprop="name" content="<?=$arItem['NAME']?>">
							<meta itemprop="description" content="<?=$arItem['NAME']?>">
						<?endif;?>
						<?if (!$bSlider):?>
							<div class="hidden compact-img lazy" <?=($bImage ? 'data-src="'.$imageSrc.'"' : 'data-src="'.$noImageSrc.'"');?> style="background-image:url('<?=\Aspro\Functions\CAsproMax::showBlankImg($bImage ? $imageSrc : $noImageSrc);?>')"></div>
						<?endif;?>
					...
					<div class="item <?=$bHalfWrapper ? '' : 'bg-white'?> <?=($bBordered ? ' bordered box-shadow rounded3' : '');?><?=(!$bImage ? ' no-img' : '');?><?=(($arResult['HAS_TITLE_FON'] == 'Y' || $useImageAlternate) ? ' long' : '');?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						<?if($bImage):?>
							<div class="image shine<?=($bLineImg ? ' pull-left' : '');?>">
								...
								<?endif;?>
									<span class="<?=(!$bBordered ? 'rounded3' : '');?> bg-fon-img lazy<?=$position;?>" data-src="<?=$imageSrc?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)" itemscope itemtype="http://schema.org/ImageObject">
										<meta itemprop="contentUrl" content="<?=$imageSrc?>">
										<meta itemprop="name" content="<?=$arItem['NAME']?>">
										<meta itemprop="description" content="<?=$arItem['NAME']?>">
									</span>
					...
					<?endif;?>
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<div class="title<?=(($bBigBlock || $arParams['ALL_BLOCK_BG']=='Y') && $arParams['TITLE_SHOW_FON'] != 'Y' ? ' font_mlg' : '');?>">
								<?if($bDetailLink):?>
									<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
								<?endif;?>
								<?=$arItem['NAME'];?>
