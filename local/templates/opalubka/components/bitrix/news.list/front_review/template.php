...
<?if($arResult['ITEMS']):?>
	...
	<div class="maxwidth-theme only-on-front">
		...
		<div class="item-views reviews<?=($bCompact ? ' compact' : '');?><?=($bMoreItem ? ' more-item' : '');?> <?=($arParams['LINKED_MODE'] == 'Y' ? ' linked ' : '');?> <?=($notSlider ? ' list-mode ' : '');?>">
			...
			<?else:?>
				<div class="owl-carousel owl-theme owl-bg-nav short-nav hidden-dots visible-nav swipeignore wsmooth1 appear-block loading_state" data-plugin-options='{"nav": true, "dots": false, "loop": true, "marginMove": false, "autoplay": false, <?=($animationSpeed >= 0 ? '"smartSpeed": '.$animationSpeed.',' : '')?> "useCSS": false, "responsive": {"0":{"items": 1, "autoWidth": false, "lightDrag": false, "margin":10, "autoplay": true}, "601":{"items": 3, "autoWidth": false, "lightDrag": false, "margin":10, "autoplay": true}, "1200":{"items": <?=$col?>, "margin":20, "nav": true}}}'>
			<?endif;?>
				<?foreach($arResult['ITEMS'] as $i => $arItem):?>
					...
					$arImage = ($bImage ? CFile::ResizeImageGet($arItem['FIELDS']['DETAIL_PICTURE']['ID'], array('width' => 70, 'height' => 10000), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : array());
					...
					?>
					<div class="item-wrapper col-md-12 col-sm-12  col-12 <?=(!$bOneItem ?  ' bg-fill-white bordered1 ' : '');?>">
						<div class="item otz jj clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered ' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<div class="top_wrapper clearfix">
								<?if($imageSrc):?>
									<div class="image center-block">
										<div class="wrap">
											...
											<?if($imageSrc):?>
												<img class="img-responsive  lazy" data-src="<?=$imageSrc?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" />
											<?endif;?>
								...
								<!--	<div class="wrap muted">
										...
										<?endif;?>
									</div>---><?if(in_array('RATING', $arParams['PROPERTY_CODE'])):?>
									...
									<?endif;?>
									<div class="title <?=($bCompact ? 'font_md' : 'font_lg');?>  title-otz">
										<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
										<?=$arItem['NAME'];?>
										<?if($bDetailLink):?></a><?endif;?>
									</div>
							...
							<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && $arItem['FIELDS']['PREVIEW_TEXT']):?>
								...
								<img src="<?=CFile::GetPath($arItem['FIELDS']['PREVIEW_PICTURE']['ID'])?>" >
