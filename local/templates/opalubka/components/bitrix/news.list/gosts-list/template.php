					...
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?if($arParams['VIEW_TYPE'] !== 'accordion'):?>
								<div class="row sid items    jjj <?=($arParams['VIEW_TYPE'] == 'table' ? 'flexbox' : '')?>">
							<?endif;?>
									...
									<?ob_start();?>
										<?if($bImage || $onlyImgPart):?>
											<div class="image <?=($bImage ? ' w-picture' : ' wo-picture wpi')?>">
                                                ...
                                                <a target="_blank" href="<?=$arItem['PROPERTIES']['FILE']['VALUE'] ? CFile::GetPath($arItem['PROPERTIES']['FILE']['VALUE']) : $imageDetailSrc?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" data-caption="<?=$arItem['NAME'];?>" class="img-inside shine">
...
<div class="d-block d-md-none item-views items-list1 <?=($onlyImgPart ? 'only-img' : '')?> <?=($documentsMode ? 'documents-mode' : '');?> <?=($licensesMode ? 'licenses-mode' : '');?> <?=$arParams['VIEW_TYPE']?> <?=$arParams['VIEW_TYPE']?>-type-block <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>">
				...
				<div class="<?=($arParams['SHOW_TABS'] == 'Y' ? 'tab-content' : 'group-content')?>" style="max-width: 100%; overflow: hidden;">
					<?// group elements by sections?>
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						<?
						// edit/add/delete buttons for edit mode
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?// show section items?>
							<?if($arParams['VIEW_TYPE'] !== 'accordion'):?>
								<div class="row sid items    jjj <?=($arParams['VIEW_TYPE'] == 'table' ? 'flexbox' : '')?> owl-carousel owl-theme owl-bg-nav short-nav hidden-dots visible-nav swipeignore wsmooth1 appear-block loading_state" data-plugin-options='{"nav": false, "dots": false, "loop": true, "marginMove": false, "autoplay": true, "smartSpeed": 600, "useCSS": true, "items": 1, "autoWidth": false, "lightDrag": false, "margin":10}'">
							<?endif;?>
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									<?
									// edit/add/delete buttons for edit mode
