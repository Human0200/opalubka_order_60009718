...
$arElement = CMaxCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), CMax::makeElementFilterInRegion($arElementFilter), false, false, array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_TEXT", "PREVIEW_PICTURE", "DETAIL_PICTURE", 'PROPERTY_FAQ', "PROPERTY_ASSOCIATED_FILTER", "PROPERTY_EXPANDABLES_FILTER", "PROPERTY_ASSOCIATED", "PROPERTY_EXPANDABLES","PROPERTY_PRIV"));

if ($arElement){
	$arElCanonical = CIBlockElement::GetList([],['IBLOCK_ID'=>$arParams["IBLOCK_ID"], 'ID'=>$arElement['ID']], false, false, ['CANONICAL_PAGE_URL'])->GetNext()['CANONICAL_PAGE_URL'];
	if (!empty($arElCanonical)) {
		$canonicalTag = '<link rel="canonical" href="'.$arElCanonical.'">';
		\Bitrix\Main\Page\Asset::getInstance()->addString($canonicalTag);
	}
}
//print_R($arElement['PROPERTY_PRIV_VALUE']);


if ($arElement['PROPERTY_PRIV_VALUE'])
{	
	...
	if (is_array($arElement['PROPERTY_PRIV_VALUE']) && in_array($id_goroda,$arElement['PROPERTY_PRIV_VALUE']))	{
