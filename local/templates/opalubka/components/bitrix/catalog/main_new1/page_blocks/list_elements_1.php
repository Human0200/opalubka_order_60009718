...
<?$GLOBALS["arLandingTizers"] = array("ID" => /*$arSeoItem["PROPERTY_TIZERS_VALUE"]*/8794646464646);?>
...
<div class="seo_block">
	<table class="order-block bordered">
		<tbody>
			<tr>
				<td class="col-md-9 col-sm-8 col-xs-7 valign"><div class="block-item">
				...
				<td class="col-md-3 col-sm-4 col-xs-5 valign btns-col"><div class="btns">
						<span><span class="btn btn-default btn-sm animate-load" data-event="jqm" data-param-form_id="ASK" data-name="question"><span>
...
<?endif;?>
<style type="text/css">
		.sketchfab-embed-wrapper{height: 1000px;}
	</style>
...
<?
//var_dump($section)
if ( $section[ "UF_NOICO" ] == "0" ) {
	?>
<div class="sect_inf" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
<?php //if ($section['UF_ADDIMG']!=""):?>
<div class="sect_pic">
	<img src="<?=CFile::GetPath($section["UF_TOPPIC"])?>" data-src="">
	<div>
		от 190 р/день
	</div>
</div>
<script type="application/ld+json">
	{
		...
		"contentUrl": "<?=CFile::GetPath($section["UF_TOPPIC"])?>",
		"datePublished": "<?=$section['TIMESTAMP_X']?>",
		"description": "Астория СЛК - <?=$section['NAME']?>",
		"name": "Астория СЛК - <?=$section['NAME']?>"
	}
	</script>
	<div class="d-none d-md-block">
		...
		<div class="r_links">
			...
			<div class="popup" style=" width: 715px; z-index:  999999;  left: 457px;top: 7% !important; border: 1px solid">
				<?=$section["UF_DOST"] ?>
				<a class="close" title="Закрыть" href="#close"></a>
			</div>
			<a href="#x" class="overlay" id="win4"></a>
			<div class="popup" style=" width: 900px; z-index:  999999;top: 357px;left: 457px;">
				<?$APPLICATION->IncludeComponent(
	...
	<div class="d-block d-md-none owl-carousel owl-theme owl-bg-nav short-nav hidden-dots visible-nav swipeignore wsmooth1 appear-block loading_state" data-plugin-options='{"nav": false, "dots": true, "loop": true, "marginMove": false, "autoplay": false, "smartSpeed": 600, "useCSS": false, "items": 1, "autoWidth": false, "lightDrag": false, "margin":10}'>
		<?=$section["UF_TECH"] ?>
		<?=str_replace(['<div class="catico">','</div></div></div>'], '', $section["UF_ICO"]) ?>
	</div>
	<?=$section["UF_TABS"]?>
	<?=$section["UF_ADV"]?>
	<?=$section["UF_AFT"]?>
...
<?}?>
<div class="1 right_block1 clearfix catalog1 <?=strtolower($arTheme["FILTER_VIEW"]["VALUE"]);?>" id="right_block_ajax">
	...
	<div class="inner_wrapper">
		...
		<?if($arSection[$arParams["SECTION_PREVIEW_PROPERTY"]]):?>
                <?
                $arContent = explode('#OBJECTS#', $arSection[$arParams["SECTION_PREVIEW_PROPERTY"]]);
                ?>
            
		<div class="group_description_block bottom muted777">
			<div>
                            <? 
                            if (count($arContent) > 1) {
                                foreach($arContent as $k => $item) {
                                    echo $item;
                                    if ($k === 0) {
                                        $APPLICATION->IncludeFile(SITE_DIR."include/custom/objects_block.php", Array(), Array());
                                    }
                                }
                            } else {
